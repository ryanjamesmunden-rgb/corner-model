...
// not been dropped, it has moved to where it does not compete: the CHIP is green when the
// card's evidence clears its bar and muted when it does not, and a weak card's accent is
// dimmed. So you can still tell at a glance which is which AND whether it is worth having.
const SIGNAL = {
  chase: "#E4E4E7",     // white — the headline signal
  mismatch: "#22D3EE",  // cyan
  streak: "#F59E0B",    // amber, matching the flame
};

function BestCard({ icon: Icon, label, title, sub, chip, onClick, strong, why, signal }) {
  const base = SIGNAL[signal] || SIGNAL.chase;
  // dim the identity colour when the evidence is thin, rather than recolouring it —
  // the card still reads as "this is the streak one", just quieter
  const accent = strong ? base : `${base}59`;   // ~35% alpha
  return (
    <button
      onClick={onClick}
    ...
    <div className="space-y-4" data-testid="home-insights">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {best?.chase ? (
            <BestCard
              icon={TrendingUp} label="Top Chase" signal="chase"
          ...
          {best?.mismatch ? (
            <BestCard
              icon={Trophy} label="Top Mismatch" signal="mismatch"
          ...
          {best?.streak ? (
            <BestCard
              icon={Flame} label="Top Streak" signal="streak"
